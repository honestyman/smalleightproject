  ...
  return(
    <div className={`w-full h-24 pt-10 fixed pb-10 z-30 ${changeColor?'drop-shadow bg-white':'none'} `}>
      <div className="w-full flex flex-row justify-between">
        ...
        <Link onClick={showNav}><TfiMenu className="mb-2 mr-2 text-3xl hidden sp:block" style={{ color: textColor }}/></Link>
        <Nav/>
      </div>
      <div className={`w-full text-center flex flex-col fixed bg-white shadow-md ${block}`}>
          <Link to={"/"} className="py-3 hover:text-[#FD6E6A] hover:transition-colors hover:bg-[#FD6E6A] hover:text-white hover:px-2">サービス</Link>
          <Link to={"/webmaru"} className="py-3 hover:text-[#FD6E6A] hover:transition-colors hover:bg-[#FD6E6A] hover:text-white hover:px-2">ウェブマる!</Link>
