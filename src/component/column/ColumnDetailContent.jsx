      ...
      <div className='w-full'>
        ...
        <div className='w-[full] flex justify-center pb-10 sp:rounded-[20px] sp:flex-wrap-reverse'>
          <div className='w-[75%] flex flex-col px-32 text-left sp:px-5 sp:w-full'>
            ...
            <p className='my-10 sp:text-xs'>{oneColumnData.description}</p>
            <div className='w-full flex justify-center py-5'>
              {oneColumnData.thumbnail && <img className='rounded hover:opacity-50 hover:scale-110' src={`${process.env.REACT_APP_BASE_URL}/img/${oneColumnData.thumbnail}`} />}
            </div>
            <div>
              {
                oneColumnData.columnfirstchildren && oneColumnData.columnfirstchildren.map((section1, index1)=>{
                  return(
                    <div key={index1}>
                      <p className='text-2xl font-bold text-[#191F4D] pt-10 sp:text-xl sp:pt-0'>{section1.title}</p>
                      <p className='my-10 sp:text-xs'>{section1.description}</p>
                      {
                        section1 && section1.columnsecondchildren.map((section2, index2)=>{
                          return(
                            <div key={index2}>
                              <p className='text-xl font-bold sp:text-[16px]'>{section2.title}</p>
                              <p className='my-10 sp:text-xs'>{section2.description}</p>
                            </div>
                          )
                        })
                      }
